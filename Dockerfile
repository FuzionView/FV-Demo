...
RUN mkdir build && cd build && \
    cmake ../mapserver \
        -DCMAKE_INSTALL_PREFIX="/opt/mapserver/8.0" \
        -DCMAKE_INSTALL_SYSCONFDIR="/opt/mapserver/8.0"/etc \
        -DWITH_KML=1 \
        -DWITH_SOS=1 \
        -DWITH_WMS=1 \
        -DWITH_FRIBIDI=0 \
    ...
    make && \
    make install && \
    mkdir /opt/mapserver/8.0/cgi-bin && \
    ln /opt/mapserver/8.0/bin/mapserv /opt/mapserver/8.0/cgi-bin && \
